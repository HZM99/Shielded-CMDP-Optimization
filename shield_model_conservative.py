...
def check_action_safety(state, action, env):
    ...
    if not hasattr(check_action_safety, 'call_count'):
        ...
        print("🛡️ Predictive Shield initialized - checking state constraints")
    ...
    # CONSERVATIVE BOUNDS: Apply 20% safety margin
    SAFETY_MARGIN = 0.2
    ...
    safe_vmax = vmax * (1 - SAFETY_MARGIN * 0.1)  # Smaller margin for voltage (2%)
    safe_vmin = vmin * (1 + SAFETY_MARGIN * 0.1)
