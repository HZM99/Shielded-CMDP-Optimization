...
class IEEE69BusEnv(SmartGridEnvironment):
    """
    IEEE 69-bus Distribution Network Environment for Reinforcement Learning
    """
    ...
    def __init__(self, ineq_penalty_weight=2.0):
        ...
        self.solver = PyPowerSolver(network_type="ieee69")
        ...
        self.nbus = 69
        self.ng = 1  # Only one generator (slack bus)

        self.genbase = self.ppc['gen'][:, 5]
        self.baseMVA = self.ppc['baseMVA']

        # Bus type identification
        self.slack = np.where(self.ppc['bus'][:, 1] == 3)[0]  # Slack bus
        self.pv = np.where(self.ppc['bus'][:, 1] == 2)[0]  # PV buses
        self.spv = np.concatenate([self.slack, self.pv])  # Slack + PV buses
        self.spv.sort()
        self.pq = np.setdiff1d(range(self.nbus), self.spv)  # PQ buses

        # Indices for different bus types
        ...
        # Reward weights
        self.we = 5.0  # EV weight
        self.wg = 1.0  # Grid weight
        ...
        self.grid_viol_weight = 12.0
        self.ev_viol_weight = 1.0

    def step(self, action):
        """Execute one time step in the environment"""
        # Clip EV actions to valid range
        ...
        # Calculate weighted constraint violation penalty
        grid_viol_sum = (
        ...
        reward -= self.ineq_penalty_weight * 0.4 * norm_ineq
    ...
    def get_action_vars(self, action):
        """Extract action variables from action vector"""
        if action.dim() == 1:
        ...
        va = action[:, 2 * self.ng + self.nbus: 2 * self.ng + 2 * self.nbus]
        pe = action[:, -self.ne:]
    ...
    def eq_resid(self, state, action):
        ...
        # Expand generator powers to all buses
    ...
    def ineq_dist_np(self, state, action):
        """Inequality constraint violations as numpy array"""
        return self.ineq_dist(state, action).detach().cpu().numpy()

    def eq_resid_np(self, state, action):
        """Equality constraint residuals as numpy array"""
