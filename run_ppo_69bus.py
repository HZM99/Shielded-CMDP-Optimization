...
def main():
    ...
    run_name = f"PPO_69Bus_{int(time.time())}"

    model = PPO("MlpPolicy", env, verbose=1, device='cuda', tensorboard_log=log_dir)
    
    training_timesteps = 100_000
    logging.info(f"--- Starting PPO Training on 69-Bus for {training_timesteps} timesteps ---")
    ...
    logging.info("--- PPO 69-BUS FINAL EVALUATION RESULTS ---")
